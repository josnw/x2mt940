<?php
 ...
 $konverterName = 'Real.de Payment CSV';
 $fileVar = 'csvfile';
 ...
 if (isset($_POST["uploadFile"]) or (isset($argv) and in_array("/csvfile", $argv))) {
	...
	if (isset($_FILES['csvfile']['tmp_name']) and (is_uploaded_file($_FILES['csvfile']['tmp_name'])))  {
		...
		$uploadFile = new myFile($docpath.'HIDE_UP_'.uniqid().".csv", "newUpload");
		$uploadFile->moveUploaded($_FILES['csvfile']['tmp_name']);
		$opdata =  new realPayment($uploadFile->getCheckedPathName());
		$opdata->importData();
	...
	$filename = $mt940data->writeToFile($docpath.'Real.de_Payment_MT940_'.date("Ymd",strtotime($parameter['startdate']))."_".uniqid().".pcc");
