<?php
 ...
 $konverterName = 'Idealo Settlement CSV';
 $fileVar = 'csvfile';
 ...
 if (isset($_POST["uploadFile"]) or (isset($argv) and in_array("/csvfile", $argv))) {
	...
	if (isset($_FILES['csvfile']['tmp_name']) and (is_uploaded_file($_FILES['csvfile']['tmp_name'])))  {
		...
		$uploadFile = new myFile($docpath.'IDEALOUP_'.uniqid().".csv", "newUpload");
		$uploadFile->moveUploaded($_FILES['csvfile']['tmp_name']);
		$opdata =  new idealoPayment($uploadFile->getCheckedPathName());
		$opdata->importData();
	...
	$filename = $mt940data->writeToFile($docpath.'IdealoPayment_MT940_'.date("Ymd",strtotime($parameter['startdate']))."_".uniqid().".pcc");
